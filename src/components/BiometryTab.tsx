...
export function BiometryTab() {
    ...
    try {
      ...
      if (farmsData && farmsData.length > 0) {
        ...
        const {
          data: pondsData,
          error: pondsError
        } = await supabase.from('ponds').select(`
            *,
            pond_batches!inner(
              id,
              current_population,
          ...
          `).eq('farm_id', farmsData[0].id).eq('status', 'in_use').eq('pond_batches.cycle_status', 'active').gt('pond_batches.current_population', 0).order('name');
        ...
        const processedPonds = pondsData?.map(pond => ({
          ...
          current_batch: pond.pond_batches[0] ? {
            ...
            latest_biometry: pond.pond_batches[0].biometrics.sort((a, b) => new Date(b.measurement_date).getTime() - new Date(a.measurement_date).getTime())[0] || null
          } : undefined
        })) || [];
  ...
  const handleBiometrySubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('biometrics').insert([biometryData]);
    ...
    try {
      ...
      if (farmsData && farmsData.length > 0) {
        const {
          data: biometryData,
          error: biometryError
        } = await supabase.from('biometrics').select(`
            *,
            pond_batches!inner(
              ponds!inner(name, farm_id),
              batches!inner(name)
            )
          `).eq('pond_batches.ponds.farm_id', farmsData[0].id).order('created_at', {
          ascending: false
        });
  ...
  const handleDeleteRecord = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from('biometrics').delete().eq('id', recordToDelete.id);
      if (error) throw error;
      toast({
  ...
  if (loading) {
    return <div className="animate-pulse space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2].map(i => <div key={i} className="h-64 bg-muted rounded"></div>)}
        </div>
      </div>;
  }
  if (ponds.length === 0) {
  ...
  return <div className="space-y-6">
      <Tabs defaultValue="active" className="w-full">
        ...
        {ponds.map(pond => {
            ...
            const biomass = batch.latest_biometry ? calculateBiomass(batch.current_population, batch.latest_biometry.average_weight) : '0.0';
            return <Card key={pond.id} className="shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-ocean)] transition-shadow">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  {batch.latest_biometry && <>
                      ...
                      {batch.latest_biometry.uniformity > 0 && <div className="flex justify-between text-sm">
                ...
                <div className="pt-4 border-t border-border">
                  <Button onClick={() => openBiometryDialog(pond)} className="w-full" size="sm">
        ...
        <TabsContent value="history" className="mt-6">
          <Card>
            ...
            <CardContent>
              {historyLoading ? <div className="animate-pulse space-y-4">
                  {[1, 2, 3].map(i => <div key={i} className="h-12 bg-muted rounded"></div>)}
                </div> : biometryHistory.length === 0 ? <div className="text-center py-8">
                  ...
                  <TableBody>
                    {biometryHistory.map(record => <TableRow key={record.id}>
                        ...
                        <TableCell>
                          {new Date(record.created_at).toLocaleDateString('pt-BR')} {new Date(record.created_at).toLocaleTimeString('pt-BR', {
                      hour: '2-digit',
                      minute: '2-digit'
                    })}
                        </TableCell>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="overflow-y-auto flex-1">
            <form id="biometry-form" onSubmit={handleBiometrySubmit} className="space-y-4 p-1">
              <div className="space-y-2">
                ...
                <Input id="measurement_date" name="measurement_date" type="date" defaultValue={getCurrentDateForInput()} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="average_weight">Peso Médio (g)</Label>
                <Input id="average_weight" name="average_weight" type="number" step="0.01" placeholder="Ex: 12.5" required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="uniformity">Uniformidade (%)</Label>
              <Input id="uniformity" name="uniformity" type="number" step="0.1" placeholder="Ex: 85.0" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="sample_size">Tamanho da Amostra</Label>
              <Input id="sample_size" name="sample_size" type="number" placeholder="Ex: 100" />
            </div>
          </form>
          </div>
          <div className="flex gap-2 pt-4 border-t bg-background flex-shrink-0">
            ...
            <Button type="submit" disabled={submitting} className="flex-1" form="biometry-form">
      ...
      <BatchBiometryModal open={showBatchDialog} onOpenChange={setShowBatchDialog} ponds={ponds} onSuccess={() => {
      loadActivePonds();
      loadBiometryHistory();
    }} />
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            {recordToDelete && <div className="bg-muted p-4 rounded-md space-y-2">
          ...
          <div className="flex gap-2 pt-4">
            ...
            <Button variant="destructive" onClick={handleDeleteRecord} disabled={submitting} className="flex-1">
