...
import { 
  ...
  BarChart3, Calculator, Target, AlertTriangle,
  ...
  const loadFinancialData = async () => {
    try {
      ...
      const [
        ...
        supabase.from('harvest_records').select('*').in('pond_batch_id', pondBatchIds),
        supabase.from('feeding_records').select('*').in('pond_batch_id', pondBatchIds),
        supabase.from('input_applications').select('*').in('pond_batch_id', pondBatchIds),
        supabase.from('operational_costs').select('*').in('farm_id', farmIds),
        supabase.from('biometrics').select('*').in('pond_batch_id', pondBatchIds).order('created_at', { ascending: false })
      ]);
      ...
      const costs: CostsByCategory[] = [
        { category: 'Ração', amount: feedCosts, percentage: (feedCosts / totalCosts) * 100, color: '#8884d8' },
        { category: 'Insumos', amount: inputCosts, percentage: (inputCosts / totalCosts) * 100, color: '#82ca9d' },
        { category: 'Pós-larvas', amount: plCosts, percentage: (plCosts / totalCosts) * 100, color: '#ffc658' },
        { category: 'Operacionais', amount: operationalCostTotal, percentage: (operationalCostTotal / totalCosts) * 100, color: '#ff7300' },
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <StatsCard
                ...
                icon={<Target />}
                variant={financialData.profitMargin >= 20 ? "success" : financialData.profitMargin >= 10 ? "primary" : "secondary"}
                subtitle="% de rentabilidade"
              />
            </div>
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {cycleFinancials.map((cycle, index) => (
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-lg">
                        {cycleFinancials.filter(c => c.status === 'active').length > 0
                          ? (cycleFinancials.filter(c => c.status === 'active').reduce((sum, c) => sum + c.roi, 0) / cycleFinancials.filter(c => c.status === 'active').length).toFixed(1)
                          : '0.0'
                        }%
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-lg">
                        {cycleFinancials.filter(c => c.status === 'completed').length > 0
                          ? (cycleFinancials.filter(c => c.status === 'completed').reduce((sum, c) => sum + c.roi, 0) / cycleFinancials.filter(c => c.status === 'completed').length).toFixed(1)
                          : '0.0'
                        }%
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-lg">
                        {financialData.totalRevenue > 0 
                          ? formatCurrency(financialData.totalCosts / (financialData.totalRevenue / priceTable))
                          : 'N/A'
                        }
                      </span>
                    </div>
