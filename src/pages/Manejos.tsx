...
export default function Manejos() {
  ...
  const {
    ...
    recentInputs,
    recentMortality
  } = useRecentManagementData(farmId);
  ...
  return <Layout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <Button variant="ghost" size="sm" onClick={() => navigate('/dashboard')} className="mb-2 bg-gradient-to-r from-slate-50 to-slate-100 hover:from-primary/10 hover:to-accent/10 border border-slate-200 hover:border-primary/20 text-slate-700 hover:text-primary transition-all duration-300">
            ...
            <div className="flex items-center gap-3 relative z-10">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {card.status === 'Novo' && <span className="px-2 py-1 text-xs font-medium bg-purple-100 text-purple-700 rounded-full ml-2 flex-shrink-0">
                            Novo
                          </span>}
                      </div>
        ...
        {!isMobile && <Card className="backdrop-blur-sm bg-card border-border hover:shadow-lg transition-all duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {recentBiometrics.length > 0 ? recentBiometrics.slice(0, 3).map((bio: any) => <div key={bio.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50 border border-border hover:bg-muted transition-all duration-300">
                        ...
                        </div>) : <p className="text-sm text-muted-foreground">Nenhum registro recente</p>}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {recentWaterQuality.length > 0 ? recentWaterQuality.slice(0, 3).map((wq: any) => <div key={wq.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50 border border-border hover:bg-muted transition-all duration-300">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {recentInputs.length > 0 ? recentInputs.slice(0, 3).map((input: any) => <div key={input.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50 border border-border hover:bg-muted transition-all duration-300">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {recentMortality.length > 0 ? recentMortality.slice(0, 3).map((mortality: any) => <div key={mortality.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50 border border-border hover:bg-muted transition-all duration-300">
